...
<div class="space-y-6 pb-24">
    ...
    <form method="POST" action="{{ route('admin.blog.update', $blog) }}" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        @csrf
        @method('PUT')
            ...
            <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
                <div class="p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none">ID Artkel</span>
                        <span class="text-xs font-mono text-gray-500">#{{ $blog->id }}</span>
                    </div>
                </div>
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="content" rows="15" required
                            class="w-full bg-gray-50/80 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 focus:bg-white dark:focus:bg-gray-900 rounded-xl outline-none transition-all duration-300 text-sm font-medium leading-relaxed">{{ old('content', $blog->content) }}</textarea>
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <select name="is_published" required 
                            class="w-full bg-gray-50/80 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 focus:bg-white dark:focus:bg-gray-900 rounded-xl outline-none transition-all duration-300 text-sm font-medium">
                            <option value="1" @selected(old('is_published', $blog->is_published) == '1')>Published</option>
                            <option value="0" @selected(old('is_published', $blog->is_published) == '0')>Draft</option>
                        </select>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
                ...
                <div class="p-6">
                    <label class="group relative flex flex-col items-center justify-center w-full min-h-[160px] border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-2xl hover:border-blue-600 transition-colors cursor-pointer overflow-hidden">
                        ...
                        <div id="image-preview-container" class="absolute inset-0 {{ $blog->cover_image ? '' : 'hidden' }}">
                            <img id="image-preview" src="{{ $blog->cover_image ? asset('storage/' . $blog->cover_image) : '#' }}" alt="Preview" class="w-full h-full object-cover">
                        ...
                        <div id="upload-placeholder" class="flex flex-col items-center justify-center p-6 text-center {{ $blog->cover_image ? 'hidden' : '' }}">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="tags" value="{{ old('tags', $blog->tags->pluck('name')->implode(', ')) }}" placeholder="tips, gadget, promo"
                            class="w-full bg-gray-50/80 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 focus:bg-white dark:focus:bg-gray-900 rounded-xl outline-none transition-all duration-300 text-sm font-medium">
                    </div>
                </div>
            </div>
        ...
        <div class="fixed bottom-0 right-0 z-[100] transition-all duration-300 bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg border-t border-gray-200 dark:border-gray-800 p-4"
            ...
            <div class="flex flex-row items-center justify-end gap-2 sm:gap-3 px-3 sm:px-6">
                <a href="{{ route('admin.blog.index') }}" class="flex-1 sm:flex-none px-4 sm:px-8 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 transition text-center whitespace-nowrap">
                    Batal
                </a>
