...
<div class="space-y-6 pb-20">
    ...
    <form method="POST" action="{{ route('admin.halaman-statis.store') }}" class="space-y-6">
        @csrf
        
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <!-- Left Side: Main Content -->
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden" x-data="{ title: '{{ old('title', '') }}', slug: '{{ old('slug', '') }}' }">
                    ...
                    <div class="p-6 space-y-5">
                        <div>
                            ...
                            <input type="text" name="title" x-model="title" required placeholder="Contoh: Tentang Kami"
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-2 px-3 py-2 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl">
                                <span class="text-[10px] text-gray-400 font-mono">{{ url('/') }}/</span>
                                <input type="text" name="slug" x-model="slug" required placeholder="tentang-kami"
                                    class="flex-1 bg-transparent border-none p-0 focus:ring-0 text-[10px] font-mono text-blue-600">
                            </div>
                        </div>

                        <div>
                            <label class="block text-[10px] font-bold text-gray-400 mb-2 uppercase tracking-widest">Ringkasan (Excerpt)</label>
                            <textarea name="excerpt" rows="2" placeholder="Deskripsi singkat untuk SEO..."
                        ...
                        <div>
                            ...
                            <textarea name="content" rows="15" required placeholder="Tulis isi halaman di sini..."
                    ...
                    <div class="p-6 space-y-5">
                        <div>
                            ...
                            <select name="is_published" required 
                                class="w-full bg-gray-50/80 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 focus:bg-white dark:focus:bg-gray-900 rounded-xl outline-none transition-all duration-300 text-sm font-medium">
                                <option value="1" @selected(old('is_published', '1') == '1')>Terbitkan (Published)</option>
                                <option value="0" @selected(old('is_published') == '0')>Simpan Draft</option>
                            </select>
                        </div>

                        <div class="pt-4 mt-4 border-t border-gray-100 dark:border-gray-800">
                            <p class="text-[10px] text-gray-400 leading-relaxed italic">
                                Halaman statis akan muncul di menu footer atau navigasi (tergantung tema). Gunakan slug yang unik untuk setiap halaman.
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <div class="fixed bottom-0 right-0 z-[100] transition-all duration-300 bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg border-t border-gray-200 dark:border-gray-800 p-4"
            ...
            <div class="flex flex-row items-center justify-end gap-2 sm:gap-3 px-3 sm:px-6">
                <a href="{{ route('admin.halaman-statis.index') }}" class="flex-1 sm:flex-none px-3 sm:px-6 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-[10px] sm:text-sm font-bold text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 transition text-center whitespace-nowrap">
                    Batal
                </a>
