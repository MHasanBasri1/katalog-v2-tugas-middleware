    ...
    <form method="POST" action="{{ route('admin.user.update', $user) }}" class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
            <div class="p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                ...
                <div class="text-right">
                    <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Terdaftar Sejak</span>
                    <span class="text-xs font-medium text-gray-900 dark:text-white">{{ $user->created_at->format('d M Y') }}</span>
                </div>
            </div>
            <div class="p-6 space-y-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div class="md:col-span-2" x-data="{ show: false }">
                        <label class="block text-[10px] font-bold text-gray-400 mb-1 uppercase tracking-widest">Ganti Password</label>
                        <p class="text-[10px] text-gray-400 italic mb-2">* Biarkan kosong jika tidak ingin mengubah password.</p>
                        <div class="relative group">
                            <input :type="show ? 'text' : 'password'" name="password" placeholder="masukkan password baru jika perlu"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2">
                    ...
                    <div>
                        ...
                        <select name="is_frozen" required 
                            class="w-full bg-gray-50/80 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 focus:bg-white dark:focus:bg-gray-900 rounded-xl outline-none transition-all duration-300 text-sm font-medium">
                            <option value="0" @selected(old('is_frozen', $user->is_frozen) == '0')>Aktif</option>
                            <option value="1" @selected(old('is_frozen', $user->is_frozen) == '1')>Dibekukan</option>
                        </select>
                    </div>
                </div>

                <div>
                    <label class="block text-[10px] font-bold text-gray-400 mb-2 uppercase tracking-widest">Alasan Bekukan / Catatan</label>
                    <input type="text" name="freeze_reason" value="{{ old('freeze_reason', $user->freeze_reason) }}" placeholder="Contoh: Spam, Melanggar aturan, dll"
                        class="w-full bg-gray-50/80 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 focus:bg-white dark:focus:bg-gray-900 rounded-xl outline-none transition-all duration-300 text-sm font-medium">
                </div>
            </div>
        </div>
        ...
        <div class="fixed bottom-0 right-0 z-[100] transition-all duration-300 bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg border-t border-gray-200 dark:border-gray-800 p-4"
            ...
            <div class="flex flex-row items-center justify-end gap-2 sm:gap-3 px-3 sm:px-6">
                <a href="{{ route('admin.user.index') }}" class="flex-1 sm:flex-none px-4 sm:px-8 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 transition text-center whitespace-nowrap">
                    Batal
                </a>
