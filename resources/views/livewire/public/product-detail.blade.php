<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    @php
        ...
        $compactSold = fn ($value) => $value >= 1000 ? floor($value / 1000) . 'k' : number_format($value);
    @endphp
    ...
    <nav class="text-sm">
        <div class="bg-white border border-gray-200 rounded-xl px-4 py-3">
            <div class="flex flex-wrap items-center gap-2 text-gray-500">
                ...
                <span class="text-gray-300">/</span>
                <a href="{{ route('kategori.detail', $product->category?->slug ?? '#') }}" class="font-semibold text-gray-600 hover:text-primary transition hover:underline">{{ $product->category?->name }}</a>
                <span class="text-gray-300">/</span>
    ...
    <section class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10">
        <div class="space-y-4">
            <div class="bg-white rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden group/gallery" x-data="productGallery(@js($galleryImages))" x-init="startAutoSlide()">
                <template x-if="slides.length">
                    <div class="relative bg-gray-50 aspect-square md:aspect-[4/3] w-full">
                        ...
                        <!-- Overlay gradient for better shadow/text -->
                        <div class="absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-black/20 to-transparent opacity-0 group-hover/gallery:opacity-100 transition-opacity duration-300"></div>
                        ...
                        <div class="absolute inset-0 flex items-center justify-between px-4 opacity-0 group-hover/gallery:opacity-100 transition-opacity duration-300">
                            <button @click="prev()" class="w-12 h-12 rounded-full bg-white/80 backdrop-blur-md text-gray-800 hover:bg-primary hover:text-white shadow-xl flex items-center justify-center transition-all duration-300 -translate-x-4 group-hover/gallery:translate-x-0 hover:scale-110">
                                <i class="fas fa-arrow-left text-sm"></i>
                            </button>
                            <button @click="next()" class="w-12 h-12 rounded-full bg-white/80 backdrop-blur-md text-gray-800 hover:bg-primary hover:text-white shadow-xl flex items-center justify-center transition-all duration-300 translate-x-4 group-hover/gallery:translate-x-0 hover:scale-110">
                ...
                <template x-if="!slides.length">
                    <div class="w-full aspect-square md:aspect-[4/3] bg-gray-50 flex flex-col items-center justify-center gap-4 text-gray-300">
                        <i class="fas fa-box-open text-6xl md:text-8xl drop-shadow-sm"></i>
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-6 md:p-8 relative overflow-hidden">
                <div class="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-bl-full -z-10"></div>
                
                <div class="mb-5 space-y-4">
                    <div class="flex items-center justify-between gap-3 flex-wrap">
                        <div class="inline-flex items-center gap-2 bg-primary/10 text-primary-dark px-3 py-1.5 rounded-lg text-xs font-bold tracking-wide">
                            <i class="fas fa-folder text-primary/70"></i> {{ $product->category?->name }} 
                        </div>

                        <button
                            type="button"
                            @auth wire:click.stop.prevent="toggleFavorite({{ $product->id }})" @else onclick="window.location.href='{{ route('user.login') }}'" @endauth
                            class="ml-auto inline-flex items-center justify-center gap-2 px-4 py-2 rounded-full border transition-all duration-300 text-xs font-bold shadow-sm {{ in_array($product->id, $favoritedProductIds ?? []) ? 'border-primary bg-primary text-white hover:bg-primary-dark hover:shadow-primary/30' : 'border-gray-200 bg-white text-gray-600 hover:text-rose-500 hover:border-rose-200 focus:outline-none' }}"
                            title="Tambah Favorit"
                        >
                            <i class="{{ in_array($product->id, $favoritedProductIds ?? []) ? 'fas fa-heart text-white animate-pulse' : 'far fa-heart' }} text-sm transition-transform group-hover:scale-110"></i>
                            Favorit
                        </button>
                    </div>
                ...
                <div class="mb-8 p-5 rounded-2xl bg-gradient-to-r from-gray-50 to-white border border-gray-100/50">
                    <div class="flex flex-col">
                        @if($product->original_price)
                            <div class="flex items-center gap-2 mb-1">
                                <span class="bg-rose-100 text-rose-600 text-[10px] sm:text-xs font-bold px-2 py-0.5 rounded">-{{ $discountPercent }}%</span>
                                <p class="text-gray-400 line-through text-xs sm:text-sm font-medium">Rp {{ number_format((float) $product->original_price, 0, ',', '.') }}</p>
                            </div>
                        @endif
                        ...
                        <div class="flex items-center gap-4 text-sm font-medium text-gray-600">
                            <span class="inline-flex items-center gap-1.5">
                                ...
                                <span class="text-gray-900 font-bold">{{ number_format((float) $product->rating_avg, 1) }}</span>
                            </span>
                            <span class="w-1.5 h-1.5 rounded-full bg-gray-300"></span>
                            <span class="inline-flex items-center gap-1.5">
                                <span class="text-gray-900 font-bold">{{ $compactSold($product->sold_count) }}</span> Terjual
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <h2 class="text-sm font-bold text-gray-800 uppercase tracking-wider flex items-center gap-2">
                        <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-blue-50 text-blue-600 shadow-inner"><i class="fas fa-store text-xs"></i></span>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        @php
                            $marketplaces = [
                                'shopee' => [
                                    ...
                                    'color' => 'bg-[#EE4D2D]',
                                    'shadow' => 'hover:shadow-[#EE4D2D]/30',
                                ],
                                'tokopedia' => [
                                    'name' => 'Tokopedia',
                                    'icon' => 'fas fa-bag-shopping',
                                    'color' => 'bg-[#42B549]',
                                    'shadow' => 'hover:shadow-[#42B549]/30',
                                ],
                                'lazada' => [
                                    'name' => 'Lazada',
                                    'icon' => 'fas fa-heart',
                                    ...
                                    'color' => 'bg-[#0f146d]',
                                    'shadow' => 'hover:shadow-[#0f146d]/30',
                                ],
                                'blibli' => [
                                    'name' => 'Blibli',
                                    'icon' => 'fas fa-shopping-basket',
                                    'color' => 'bg-[#0095DC]',
                                    'shadow' => 'hover:shadow-[#0095DC]/30',
                                ],
                                'tiktok shop' => [
                                    'name' => 'TikTok Shop',
                                    'icon' => 'fab fa-tiktok',
                        ...
                        @foreach($marketplaceLinks as $key => $url)
                            @if(isset($marketplaces[$key]))
                                <a href="{{ $url }}" target="_blank" rel="noopener noreferrer" class="group relative overflow-hidden inline-flex items-center justify-center gap-2.5 rounded-2xl px-4 py-3.5 font-bold text-sm {{ $marketplaces[$key]['color'] }} text-white hover:-translate-y-1 hover:shadow-lg {{ $marketplaces[$key]['shadow'] }} transition-all duration-300">
                                    <span class="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></span>
                                    <i class="{{ $marketplaces[$key]['icon'] }} {{ $marketplaces[$key]['icon_class'] ?? '' }} relative z-10 text-base"></i> 
                                    <span class="relative z-10">{{ $marketplaces[$key]['name'] }}</span>
                                </a>
                            @else
                                <a href="{{ $url }}" target="_blank" rel="noopener noreferrer" class="group relative overflow-hidden inline-flex items-center justify-center gap-2.5 rounded-2xl px-4 py-3.5 font-bold text-sm bg-gray-600 text-white hover:-translate-y-1 hover:shadow-lg hover:shadow-gray-600/30 transition-all duration-300">
                                    <span class="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></span>
                                    <i class="fas fa-store relative z-10"></i> <span class="relative z-10 font-bold uppercase">{{ $key }}</span>
                                </a>
                            @endif
                        @endforeach
                    </div>
    ...
    <section class="mt-12">
        <div class="flex items-center justify-between mb-8 border-b border-gray-100 pb-4">
            <div>
                <h2 class="text-2xl md:text-3xl font-black text-gray-900 tracking-tight flex items-center gap-3">
                    Produk Terkait
                </h2>
                <span class="text-sm text-gray-500 mt-1 block">Rekomendasi lain untuk kategori {{ $product->category?->name }}</span>
            </div>
            
            <a href="{{ route('katalog') }}" class="text-sm font-semibold text-primary hover:text-primary-dark transition-colors hidden sm:block">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-5 pb-8">
            @forelse($relatedProducts as $item)
                ...
                <div class="bg-white rounded-3xl p-3 border border-gray-100 shadow-[0_2px_10px_rgb(0,0,0,0.02)] hover:shadow-[0_10px_30px_rgb(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300 flex flex-col group h-full relative">
                    <a href="{{ route('produk.detail', $item->slug) }}" class="block bg-gray-50 rounded-2xl aspect-[4/4] w-full overflow-hidden mb-4 relative">
                        ...
                        <div class="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                            <button 
                                type="button"
                                wire:click.stop.prevent="toggleFavorite({{ $item->id }})" 
                                class="w-8 h-8 md:w-9 md:h-9 bg-white/90 backdrop-blur-md rounded-full flex items-center justify-center shadow-lg hover:bg-primary hover:text-white transition-colors duration-300 text-gray-600 {{ in_array($item->id, $favoritedProductIds ?? []) ? 'text-rose-500' : '' }}"
                            >
                                <i class="{{ in_array($item->id, $favoritedProductIds ?? []) ? 'fas' : 'far' }} fa-heart text-[13px] md:text-sm"></i>
                            </button>
                        </div>
                    </a>
                    ...
                    <div class="px-1 flex flex-col flex-1">
                        ...
                        <div class="mt-auto">
                            <p class="text-primary font-black text-base md:text-xl leading-tight">Rp {{ number_format((float) $item->price, 0, ',', '.') }}</p>
                            @if($item->original_price)
                                <p class="text-gray-400 line-through text-[10px] md:text-sm mt-0.5 decoration-gray-300">Rp {{ number_format((float) $item->original_price, 0, ',', '.') }}</p>
                            @endif
                        </div>
            ...
            @empty
                <div class="col-span-full py-12 text-center bg-gray-50 rounded-3xl border border-gray-100">
                    <p class="text-gray-500 font-medium">Belum ada produk terkait pada kategori ini.</p>
                </div>
            @endforelse
        </div>
    </section>
