...
<head>
    ...
    <meta name="author"
        content="@yield('meta_author', $setting->seo_settings['author'] ?? $setting->shop_name ?? 'Kataloque')">
    <link rel="canonical" href="@yield('canonical', url()->current())">
    <meta name="theme-color" content="#2563eb">

    {{-- Search Console Verifications --}}
    @if($setting && isset($setting->seo_settings['google_verification']))
    <meta name="google-site-verification" content="{{ $setting->seo_settings['google_verification'] }}"> @endif
    @if($setting && isset($setting->seo_settings['bing_verification']))
    <meta name="msvalidate.01" content="{{ $setting->seo_settings['bing_verification'] }}"> @endif
    @if($setting && isset($setting->seo_settings['yandex_verification']))
    <meta name="yandex-verification" content="{{ $setting->seo_settings['yandex_verification'] }}"> @endif

    {{-- Open Graph / Facebook --}}
    <meta property="og:type" content="@yield('og_type', 'website')">
    <meta property="og:site_name" content="{{ $setting->shop_name ?? 'Kataloque' }}">
    <meta property="og:title"
        content="@yield('og_title', trim($__env->yieldContent('title', ($setting->seo_settings['seo_title'] ?? $setting->shop_name ?? 'Kataloque'))))">
    <meta property="og:description"
        content="@yield('og_description', trim($__env->yieldContent('meta_description', $setting->shop_description ?? 'Kataloque adalah katalog produk modern.')))">
    <meta property="og:url" content="@yield('og_url', trim($__env->yieldContent('canonical', url()->current())))">
    <meta property="og:image"
        content="@yield('og_image', ($setting->seo_settings['og_image'] ?? 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?q=80&w=1200&h=630&auto=format&fit=crop'))">
    <meta property="og:locale" content="id_ID">

    {{-- Twitter --}}
    <meta name="twitter:card"
        content="@yield('twitter_card', $setting->seo_settings['twitter_card'] ?? 'summary_large_image')">
    <meta name="twitter:title"
        content="@yield('twitter_title', trim($__env->yieldContent('title', ($setting->seo_settings['seo_title'] ?? $setting->shop_name ?? 'Kataloque'))))">
    <meta name="twitter:description"
        content="@yield('twitter_description', trim($__env->yieldContent('meta_description', $setting->shop_description ?? 'Kataloque adalah katalog produk modern.')))">
    <meta name="twitter:image"
        content="@yield('twitter_image', trim($__env->yieldContent('og_image', ($setting->seo_settings['og_image'] ?? 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?q=80&w=1200&h=630&auto=format&fit=crop'))))">
    ...
    @if($setting && isset($setting->system_settings['google_analytics_id']))
        <script async
            src="https://www.googletagmanager.com/gtag/js?id={{ $setting->system_settings['google_analytics_id'] }}"></script>
        <script>
...
<body
    class="m-0 min-h-screen text-gray-800 font-['Plus_Jakarta_Sans'] antialiased overflow-x-hidden relative selection:bg-primary/10 selection:text-primary">
    {{-- Announcement Bar Integration --}}
    @if($setting && ($setting->system_settings['announcement_enabled'] ?? false))
        <div
            class="relative bg-gradient-to-r from-amber-500 to-amber-600 text-white p-2.5 text-center overflow-hidden z-[200]">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-center gap-3">
                <span class="inline-flex items-center gap-2 group cursor-default">
                    ...
                    <span
                        class="text-[11px] font-black uppercase tracking-widest">{!! $setting->system_settings['announcement_text'] !!}</span>
                </span>
                @if(!empty($setting->system_settings['announcement_url']))
                    <a href="{{ $setting->system_settings['announcement_url'] }}"
                        class="inline-flex items-center gap-1.5 bg-white/20 hover:bg-white/30 backdrop-blur-md px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest transition-all duration-300 hover:scale-105 active:scale-95 border border-white/20">
    ...
    <a href="#main-content"
        class="sr-only focus:not-sr-only focus:absolute focus:top-0 focus:left-0 focus:z-[9999] focus:p-4 focus:bg-white focus:text-primary"
    ...
    @if(auth()->check() && auth()->user()->hasRole('admin'))
        <div
            class="fixed top-0 left-0 right-0 z-[200] bg-gray-900 text-white text-[11px] font-bold py-1.5 px-4 backdrop-blur-md border-b border-white/10 flex justify-between items-center h-[32px]">
            <div class="flex items-center gap-4">
                <div
                    class="flex items-center gap-2 px-2 py-0.5 bg-rose-500 rounded text-[9px] uppercase tracking-wider shadow-lg shadow-rose-500/20">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.dashboard') }}"
                    class="hover:text-primary transition-colors flex items-center gap-1.5 px-3 py-1 rounded-md hover:bg-white/5">
    ...
    <div
        class="md:hidden fixed bottom-0 left-0 right-0 z-[90] bg-white/95 mobile-nav-blur border-t border-gray-100 px-4 py-2 pb-safe shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
        <div class="flex items-center justify-around gap-2" role="navigation" aria-label="Menu Navigasi Mobile">
            <a href="/"
                class="flex flex-col items-center gap-1 p-2 {{ request()->routeIs('home') ? 'text-primary' : 'text-gray-500' }}"
    ...
    <div x-data="{ 
        ...
        class="fixed top-24 left-1/2 -translate-x-1/2 z-[200] w-[90%] max-w-sm pointer-events-none" x-cloak>
        <div :class="{
